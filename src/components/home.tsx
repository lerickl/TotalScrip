...
import ViewResponseGpt from './viewResponse'
...
import AddSupabase from '../apis/dbsupabase.ts'
import '../styles/App.css'
import { Database } from '../apis/types/database'
...
export default function Home() {
  ...
  const  CloseModalContinue = ()=> {
    const  xadata: Database['public']['Tables']['DataPromt']['Insert'] = { 
  ...
  const handleSubmit =  (event: { preventDefault: () => void } ) => {
    ...
    if (formData.quest1.trim() === '') {
      alert('Por favor, complete el campo 1: "¿Qué producto o servicio estás ofreciendo?"')
      const quest1Input = document.getElementsByName('quest1')[0] as HTMLInputElement
      quest1Input.focus()
      return
    }
  
    if (formData.quest2.trim() === '') {
      alert('Por favor, complete el campo 2: "Describe brevemente tu producto o servicio."')
      const quest2Input = document.getElementsByName('quest2')[0] as HTMLInputElement
      quest2Input.focus()
      return
    }
  
    if (formData.quest3.trim() === ''  ) {
      alert('Por favor, complete el campo 3 con un precio válido (ejemplo: 300USD).')
      document.getElementsByName('quest3')[0].focus()
      return
    }
  
    if (formData.quest4.trim() === '') {
      alert('Por favor, complete el campo 4: "¿Qué oferta especial puedes proporcionar?"')
      document.getElementsByName('quest4')[0].focus()
      return
    }

    if (formData.quest5.trim() === '') {
      alert('Por favor, complete el campo 5: "Escribe el nombre de tu empresa."')
      const quest5Input = document.getElementsByName('quest5')[0] as HTMLInputElement
      quest5Input.focus()
      return
    }
    const dialogElement = document.getElementById("DialogFinal") as HTMLDialogElement
    if (dialogElement) {
      dialogElement.showModal()
    }
    
  }
  const CerrarDialogEnviarPromt = () => {
    const dialogElement = document.getElementById("DialogFinal") as HTMLDialogElement
    if (dialogElement) {
      dialogElement.close()
    }
  }
  const handleSubmitPromt = async (event: FormEvent<HTMLFormElement> ) => {
    event.preventDefault()
    const data =CloseModalContinue()
    AddSupabase(data)
    const dialogElement = document.getElementById("DialogFinal") as HTMLDialogElement
    if (dialogElement) {
      dialogElement.close()
    ...
    <>
      ...
      <dialog className='modalCard' id='miDialog' >
        ...
        <section className='cardBienvenido'>
          <div className='contentCardBienvenido' >
        ...
        (<section className='preguntas'  >
          <section className='quests'>
            ...
            <p> Para productos {formTicket} Ticket, te recomendamos vender {recomendacion} 
            </p>
            <form onSubmit={handleSubmitPromt} >
              <div>
              ...
              <button onClick={handleSubmit}>Crear Guion</button>
              <dialog  className='modalCardEnviar' id='DialogFinal'>
                <section  className='modalEmail'  >
                  <div   >       
                    <a onClick={CerrarDialogEnviarPromt}>{'<'}atras</a>     
                    <h2>Escribe tu correo para acceder al script</h2>
                    <label htmlFor='email'>Ingres tu Email</label>
                    <input type='email' placeholder='Email' value={emailScript} onChange={handlechangeEmailScript}  required/>
          ...
          <p >creando guion por favor espera unos segundos....</p>
